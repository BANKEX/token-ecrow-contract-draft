...
contract TokenEscrow is usingOraclize {
	...
	function TokenEscrow() {
		...
		// Discount policy
		tokenSupplies[0] = TokenSupply(1000000, 0, 20); // First million of tokens will go for price of 20 cents each
		tokenSupplies[1] = TokenSupply(2000000, 0, 30); // The rest (two million of tokens) will go for price of 30 cents each
	...
	/**
	 * @dev Approves Presale further selling from specifed buyer to specifed recipient
	 * @param buyer Account to get money/ether from
	 * @param recipient Account to send Presale tokens to
	 */	
	function approve(address buyer, address recipient) owneronly {
		escrows[buyer] = Escrow(owner, recipient);
	}

	/**
	 * @dev Approves Presale further selling for specifed buyer
	 * @param buyer Account to get money/ether from and send Presale tokens to
	 */	
	function approve(address buyer) owneronly {
		approve(buyer, buyer);
	}
	...
	function() payable {
		...
		// Do not allow payments from non-approved buyers
		if (escrow.seller == 0)
			throw;
